...
function verificacao(numb:number):string {
    if (numb % 2 == 0) {
        return numb+"É par";
    } else if (numb % 2 == 1){
        return numb+"É impar";
    } else{
        return "Algo deu errad"
    }
};


//ex5 - Criar uma função que retorna o maior entre dois números.
function maior(a1:number, b2:number):string{
    if (a1 > b2){
...
function calcularmedia(n1:number, n2:number, n3:number){
    return((n1+n1+n3)/3)
}
...
function conversaoFahrenheit(n1:number){
    return((n1*1.8)+32)
}
